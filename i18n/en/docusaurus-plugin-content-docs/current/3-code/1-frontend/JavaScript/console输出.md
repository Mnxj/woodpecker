...
//
var a = {x: 1}
var b = a // copies the reference to a; b = {x:1}
a.x = a = {n: 1} // a = {n:1}; a.x = a, so a.x = {n:1}.
console.log(a) // {n:1}
console.log(b)// {x:{n:1}}
...
function find(obj, str){
    ...
    let current = obj;
    // Walk the path; access properties step by step
    for (var i = 0; i < keys.length; i++) {
        // If current object doesn't have the property, return undefined
...
function test() {
    list.forEach(async x => { // synchronous code
        const res = await square(x);
        console.log(res);
    });
}
test() // wait 1000 → 1 4 9
//
<style>
.b { color: red;}
.a { color: blue;}//1
