### Given a promise and a time, return its state if it settles before the time, otherwise return a new promise rejected with `new Error()`
    ...
    return new Promise((resolve, reject) => {
        ...
        const response = (i, value) => {
            if (value instanceof Promise) {
                // Note: `val` here is the resolved value, not the outer `value`
                value.then((val) => {
                    response(i, val)
                }, (err) => {
                    result[i] = {
...
// 1. Non-Promise, non-thenable
Promise.resolve(1).then(console.log) // 1
// 2. Promise that fulfills
